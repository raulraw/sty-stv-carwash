...
function useGoogleReviews() {
  ...
  useEffect(() => {
    fetch('/api/reviews')
      ...
      .then(data => {
        if (data.reviews?.length) { setReviews(data.reviews); setIsLive(true); }
...
// ─────────────────────────────────────────────
// AVATAR CU INIȚIALE + CULORI UNICE
// ─────────────────────────────────────────────
...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div
      className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 border border-white/10 text-white font-bold text-[15px] tracking-wider select-none"
      style={{
...
function Stars({ rating, size = 'sm' }: { rating: number; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className="flex items-center gap-0.5" aria-label={`Rating: ${rating} din 5 stele`}>
      {[1, 2, 3, 4, 5].map(i => (
        <svg
          key={i}
          width={px}
          height={px}
          viewBox="0 0 24 24"
          fill={i <= rating ? '#e30613' : 'rgba(255,255,255,0.15)'}
          aria-hidden="true"
        >
...
function RatingBar({ label, count, total }: { label: string; count: number; total: number }) {
  ...
  return (
    <div ref={ref} className="flex items-center gap-3 group">
      <span
        className="text-white/50 text-[12px] w-5 text-right flex-shrink-0"
      ...
      {/* Count */}
      <span
        className="text-white/35 text-[11px] w-7 text-right flex-shrink-0 tabular-nums"
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', maskImage: 'linear-gradient(to top, black 0%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to top, black 0%, transparent 100%)' }}
        />
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-black to-transparent pointer-events-none z-20" />

        {/* Fundal subtil — glow roșu centrat */}
        <div
        ...
        <div className="relative z-30 max-w-6xl mx-auto px-6">
          ...
          <div ref={titleRef} className="text-center mb-14">
            ...
            {isLive && (
              ...
              >
                ...
                <span
                  className="text-emerald-300 text-[12px] font-semibold tracking-wider"
                  style={{ fontFamily: "'Outfit', sans-serif" }}
                >
                  Live · Google Reviews
                </span>
          ...
          <div className="grid lg:grid-cols-[300px_1fr] gap-8 items-start">
            ...
            <div>
              {reviews.length > 0 ? (
                <>
                  {/* Desktop — carusel 3 carduri pe un rând */}
                ...
                <div className="flex items-center justify-center h-48 text-white/30 text-sm" style={{ fontFamily: "'Nunito', sans-serif" }}>
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/[0.04] border border-white/[0.08]">
              ...
              <span
                ...
                style={{ fontFamily: "'Outfit', sans-serif" }}
              >
                Recenzii colectate de pe <strong className="text-white/45 font-semibold">Google Maps</strong>
              </span>
            </div>
