...
// Dynamic import pentru Masonry (componenta grea)
const Masonry = dynamic(() => import('./animations/Masonry'), {
  ssr: false,
  loading: () => (
    <div className="w-full min-h-[700px] md:min-h-[850px] bg-black/30 rounded-3xl flex items-center justify-center">
      <span className="text-white/30">Se încarcă galeria...</span>
    </div>
  ),
});
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="relative w-full min-h-[700px] md:min-h-[850px]">
            <Masonry
              items={galleryItems}
              ease="power3.out"
              duration={0.7}
          ...
          <p className="text-center mt-12 text-white/40 text-sm tracking-wide" style={{ fontFamily: "'Outfit', sans-serif" }}>
...
// Lightbox Component
function LightboxContent({ selectedIndex, direction, onClose, onPrev, onNext }: any) {
  return (
    <>
      <motion.div
        initial={{ opacity: 0 }}
      ...
      <motion.div
        ...
        className="fixed inset-0 z-50 flex items-start justify-center pt-20 sm:pt-28 p-4 sm:p-8 pointer-events-none"
      >
        <div
          className="relative pointer-events-auto w-full max-w-4xl rounded-[2rem] overflow-hidden bg-white/[0.08] backdrop-blur-3xl border border-white/[0.18] shadow-2xl shadow-black/60"
        ...
        >
          ...
          <div className="absolute top-0 left-8 right-8 h-px z-20 pointer-events-none" style={{ background: 'linear-gradient(90deg, transparent, rgba(227,6,19,0.7), transparent)' }} />
          ...
          <div className="relative z-30 flex items-center justify-between px-6 pt-5 pb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-white/70 text-[13px] font-semibold tracking-[0.15em] uppercase">Sty &amp; Stv Carwash</span>
            </div>

            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-white/[0.08] border border-white/[0.14]">
              ...
              <motion.button onClick={onClose} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.93 }} className="w-9 h-9 rounded-xl bg-[#e30613]/15 border border-[#e30613]/35 flex items-center justify-center">
          ...
          <div className="relative z-30 flex items-center justify-between px-6 pb-6">
            <div className="flex-1 mr-6 h-[3px] rounded-full bg-white/[0.08] overflow-hidden">
              <motion.div className="h-full rounded-full bg-[#e30613]" animate={{ width: `${((selectedIndex + 1) / TOTAL) * 100}%` }} />
            </div>
            <div className="flex items-center gap-3">
